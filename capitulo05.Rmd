...
#knitr::knit_hooks$set(document = function(x) {sub('\\usepackage{color}', '\\usepackage[usenames,dvipsnames]{xcolor}', x, fixed = TRUE)})
...
Un "expresión R en línea" o el interior de un párrafo comienza con `` `r `` y finaliza con un acento abierto `` ` ``.
...
\\newline{}Fuente: elaboración propia",fig.pos="H"}
knitr::include_graphics("graficos/capitulo05ejemplo01.png")
```
...
**Nota**. Se recomienda el uso de la opción **`fig.pos="htbp"`** o **`fig.pos="!htbp"`** para que no aparezcan espacios en blanco adicionales (`!` obliga). Se puede obligar a que en el caso que no se especifique se comporte obligadamente como se indiquemos con `floatplacement{figure}{}` o `floatplacement{table}{}` . Por ejemplo, si queremos obligar que las figuras se coloquen como: "!bthp".
...
El siguiente código ilustra cómo crea un fichero con ayuda del paquete "ggplot2" y lo grabamos en un fichero ".png" con ayuda de la función **`ggsave()`**.
...
fig.cap="\\label{fig:cap05gg02}Gráfico de Líneas creado con ggplot2. 
...
```{r echo=FALSE,out.width='80%',fig.align='center',fig.pos='htbp',fig.cap="\\label{fig:cap05gg02}Gráfico de Líneas creado con ggplot2. Fuente: elaboración propia"}
library(ggplot2)
ggplot(mtcars, aes(mpg, wt)) + 
  geom_line(col="blue")
```
...
fig.cap='Dos gráficos', fig.subcap = c('Regresión', 
'Gráfico sobre cars'),fig.pos="htbp"}
...
```{r plot-cars, fig.height = 3, fig.width = 4, out.width='49%', fig.cap='Dos gráficos', fig.subcap = c('Regresión', 'Gráfico sobre cars'),fig.pos="htbp"}
plot(mpg ~ wt, data = mtcars)
plot(cars)
```
...
Otro ejemplo en el que solamente se usa leyenda para los dos gráficos (también se modifican los márgenes). Importante el uso de la opción de chunk: **fig.show="hold"**.


```{r out.width='45%',fig.show="hold",fig.cap="Dos gráficos R cara a cara",fig.pos="htbp"}
...
## Cómo incluir un tabla o cuadro con información
...
- **`caption="\\label{}Explicación..."`**: Añade una leyenda que explique el contenido de la tabla junto a un identificador para hacer referencia a ella **`\ref{}`**.
...
Se presentan dos ejemplos de tablas apaisadas.

1. La tabla \ref{tab:apaisada} tiene tiene de especial que se usa la función **`landscape()`**.
...
text_tbl %>%
  kable(booktabs = TRUE,caption="\\label{tab:apaisada}Esta tabla 
          está apaisada y en una página separada") %>% 
      column_spec(c(1), 
          width = "2cm") %>% 
      column_spec(c(2), 
          width = "10cm") %>% 
...
Con el paquete babel y la opción "spanish" (o al final del fichero: "latex/latex_preambulo.tex"):
...
En el siguiente enlace [overleaf: alfabeto griego y símbolos matemáticos](https://www.overleaf.com/learn/latex/List_of_Greek_letters_and_math_symbols)
...
Algunos de las referencias bibliográficos incluidas en el fichero "bib/library.bib" aparecen a continuación. Se recogen ejemplos de publicaciones de tipos diferentes: "Manual", "book", "article", "misc", pero existen muchas más.
...
En el texto se podrán incluir citas a cualquiera de las referencias bibliográficas incluidas en los ficheros de bibliografía ".bib" con ayuda del código LaTeX: **`\cite{idbibliografico}`** y sus variantes.
...
@Luque2017, [@R-base] y @Wickham2017(págs.3-7), 
o también se podría ver en @Luque2017 y @Wickham2017.
```
...
**Importante**. En la bibligrafía  que **se muestra al final del documento**  aparecerán las referencias bibliográficas de aquellas que hayan sido citadas, es decir, hayan aparecido en un comando **`@identificador`**.
...
### Estilos bibliográficos de presentación

En la cabecera "yaml" del fichero "tfe_principal.Rmd" aparece la clave "csl:", que permite presentar la bibliografía siguiente las pautas que se exigen en algunas revistas científicas.
...
<!-- ```latex -->
<!-- \bibliographystyle{nombreestilo} -->
<!-- ``` -->
<!-- donde "nombreestilo" puede ser uno de los siguientes valores:  -->
...
**Nota**: hay otra forma de manejar bibliografías con el paquete [biblatex](https://www.overleaf.com/learn/latex/Bibliography_management_with_biblatex) que posee características adicionales al sistema bibtex.

## Otras cuestiones

### Cómo hacer referencias a otras partes del documento

Se pueden hacer referencia a capítulos, secciones, subsecciones, u otros subniveles. Para ello es conveniente saber cómo insertar identificadores a estas partes. Se incluir un identificador,
...
**Nota**. No se pueden asociar identificadores iguales a secciones distintas.
...
El siguiente código se utiliza varios comandos LaTeX para producir cajas coloreadas:
...
En el siguiente ejemplo se insertan en nuestro documento las páginas 8 y 14 del fichero local “xaringanej01_defecto.pdf” con un escalado de 75% (de la 13 a 14, habría que escribir: pages=13-14):
